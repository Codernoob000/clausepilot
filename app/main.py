...
class RedlineExportRequest(BaseModel):
    subject: str = "Counter-Proposal Redline"
    body: str = ""
    session_id: str = "anonymous"


@app.post("/api/export_redline_docx")
def export_redline_docx(req: RedlineExportRequest):
    """Generates and streams a genuine Microsoft Word (.docx) redline document."""
    try:
        import docx
        doc = docx.Document()
        doc.add_heading("ClausePilot — Counter-Proposal Redline", 0)
        
        meta_p = doc.add_paragraph()
        meta_p.add_run(f"Session Reference: ").bold = True
        meta_p.add_run(f"{req.session_id}\n")
        meta_p.add_run(f"Subject Agenda: ").bold = True
        meta_p.add_run(f"{req.subject}\n")
        
        doc.add_heading("Proposed Redline & Negotiation Draft", level=1)
        ...
        doc.add_paragraph("Generated by ClausePilot. This document is provided for informational and negotiation support purposes only and does not constitute formal legal counsel.")
        
        bio = io.BytesIO()
        doc.save(bio)
        bio.seek(0)
        
        safe_session = "".join(c for c in req.session_id if c.isalnum())[:8] or "session"
        filename = f"ClausePilot_Counter_Proposal_{safe_session}.docx"
